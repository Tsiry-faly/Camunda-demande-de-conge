...
export default function AdminApprovalPage() {
  ...
  async function traiter(demande, decision) {
    ...
    try {
      ...
      // Retrait immediat de la liste locale : on n'attend pas le re-fetch,
      // qui peut renvoyer temporairement l'ancien statut (delai d'indexation
      // cote Camunda entre la completion de la tache et sa recherche).
      setDemandes((prev) => prev.filter((x) => x.id !== demande.id))
    } catch (err) {
      console.error(err)
